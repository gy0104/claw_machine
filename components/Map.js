...
export default function KakaoMap() {
  ...
  const getClosestSpots = (centerLatLng) => {
    const spotsWithDistance = locations
      .filter((loc) => loc.lat && loc.lng)
      .map((loc) => {
        const dist = centerLatLng.getDistance(
      ...
      .then((text) => {
        ...
        script.onload = () => {
          window.kakao.maps.load(() => {
            ...
            // 주소 → 좌표 변환 후 마커
            initialData.forEach((loc, idx) => {
              geocoderRef.current.addressSearch(loc.address, (result, status) => {
                if (status === window.kakao.maps.services.Status.OK) {
                  const lat = parseFloat(result[0].y);
                  const lng = parseFloat(result[0].x);

                  const updated = { ...loc, lat, lng };
                  setLocations((prev) => {
                    const newData = [...prev];
                    newData[idx] = updated;
                    return newData;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <div onClick={goToMyLocation} style={{ position: 'absolute', bottom: 20, right: 20, width: 48, height: 48, background: 'white', borderRadius: '50%', boxShadow: '0 2px 6px rgba(0,0,0,0.2)', display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', zIndex: 10 }}>
        <img src="/icons/locate-me.png" width="24" height="24" alt="내 위치" />
      </div>
